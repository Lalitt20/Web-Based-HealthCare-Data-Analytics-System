...
if (isset($_POST['submit'])) {
	if (isset($_GET['editid'])) {
		$sql = "UPDATE appointment SET 
					patientid='$_POST[select4]',
					departmentid='$_POST[select5]',
					appointmentdate='$_POST[appointmentdate]',
					appointmenttime='$_POST[time]',
					doctorid='$_POST[select6]',
					status='$_POST[select]' 
		...
		$sql = "INSERT INTO appointment(patientid, departmentid, appointmentdate, appointmenttime, doctorid, status)
				VALUES('$_POST[select4]', '$_POST[select5]', '$_POST[appointmentdate]', '$_POST[time]', '$_POST[select6]', '$_POST[select]')";
		if ($qsql = mysqli_query($con, $sql)) {
			$appointment_id = mysqli_insert_id($con);
			$doctorid = $_POST['select6'];

			// Fetch consultation charge from doctor table
			$fee_sql = mysqli_query($con, "SELECT consultancy_charge FROM doctor WHERE doctorid='$doctorid'");
			$fee_row = mysqli_fetch_array($fee_sql);
			$consultancy_charge = $fee_row['consultancy_charge'];

			// Create billing entry
			mysqli_query($con, "INSERT INTO billing (appointmentid, billingdate, billingtime, discount)
								VALUES ('$appointment_id', CURDATE(), CURTIME(), 0)");
			...
			mysqli_query($con, "INSERT INTO billing_records (billingid, bill_type, bill_type_id, bill_amount, bill_date)
								VALUES ('$billing_id', 'Consultancy Charge', '$doctorid', '$consultancy_charge', CURDATE())");

			echo "<script>alert('Appointment and billing created successfully.');</script>";
